...
class CreateEndpointsFromProto extends Command
{
    ...
    protected ?string $name = 'frock:create-endpoints-from-proto';
...
//            ->setProtected();
    ...
    public function abstractEndpointMethodCreate(ClassType $innerGrpcController, \Nette\PhpGenerator\Method $method, string $transportFullName): void
    {
        ...
        $innerControllerMethod->setBody(
            ...
            '$transportController->context = $ctx;' . "\n" .
...
//            '$this->callDurationHistogramMetric->observe(microtime(true) - $timeStart, [\''.$metricLabel.'\']);'."\n".
    ...
    private function upgradeInterface(string $interfaceFilePath) {
        $interfaceContent = file_get_contents($interfaceFilePath);
        $namespace = new PhpNamespace($this->getGrpcInterfaceNamespace($interfaceContent));
        $uses = [];
        preg_match_all('/use\s+(.*);/', $interfaceContent, $uses);
        foreach ($uses[1] as $use) {
            if (!str_contains($use, 'Spiral')) {
                $namespace->addUse($use);
            }
        }

        /** @var InterfaceType $interface */
        $interface = InterfaceType::fromCode($interfaceContent);
        $namespace->add($interface);

        foreach ($interface->getMethods() as $method) {
            $params = $method->getParameters();
            $params['ctx']->setType('array');
            $method->setComment('');
        }

        $interface->setExtends([]);
        $this->putNamespaceToFile($namespace, $interfaceFilePath);
            ...
            foreach (scandir('/var/www/php/protoGenerated/'.$serviceDir) as $subServiceDir) {
                ...
                foreach (scandir('/var/www/php/protoGenerated/'.$serviceDir.'/'.$subServiceDir) as $versionDir) {
                    ...
                    foreach ($interfacePaths as $interfacePath) {
                        $this->upgradeInterface($interfacePath);
